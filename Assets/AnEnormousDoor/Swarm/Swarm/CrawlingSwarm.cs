...
namespace Swarm
{
    public class CrawlingSwarm : MonoBehaviour
    {
        ...
        [SerializeField] protected AnimationCurve pauseCurve;
        [SerializeField] protected float pauseThreshold = .5f;
        [SerializeField] protected float pauseFrequency = 2;
        ...
        void Update()
        {
            ...
            // && pauseCurve.Evaluate(Time.time % pauseFrequency / pauseFrequency) > pauseThreshold
            if (delta > 0)
            {
                ...
                _compute.SetFloat("Time", Time.time * 2);
